...
async def handle_query(request: Request):
    try:
        ...
        # Unified mode - no mode separation needed
        ...
        # Build help context for all queries
            top_docs = search_documents(query)
            context = build_context([doc for _, doc in top_docs])

        # --- Unified system prompt with all capabilities ---
            system_prompt = (
            "You are Cubie, a helpful AI assistant for TCube. You can help with both analytics and application questions.\n\n"
            "ANALYTICS CAPABILITIES:\n"
            "Use the database schema below to write read-only SQL queries when users ask for data analysis.\n"
                "Never modify data. Always wrap SQL in a function call if needed.\n\n"
            f"{DB_SCHEMA}\n\n"
            "HELP CAPABILITIES:\n"
            "Answer questions about TCube application features using the provided help documentation.\n\n"
            "UNIFIED RULES:\n"
            ...
            "• Before drafting any email, FIRST compute the required metrics via tools and include concrete numbers in the draft (no placeholders).\n"
            ...
            "• For help questions, provide clear answers and include relevant links when available.\n"
            "• Always be helpful and conversational, regardless of whether the question is about analytics or application help.\n"
            "• Always use a polite, friendly, and conversational tone.\n"
            "• If the user asks for humor (e.g., jokes), respond playfully.\n"
            "• When giving instructions, always use bullet points (-) or numbered lists (1., 2., ...) with spacing.\n"
            "• When referencing links: Use [descriptive link text](URL) instead of raw URLs.\n"
            "• Do not repeat greetings in each response.\n"
            "• If you don't know the answer, say so politely.\n"
            "• Responses must always be formatted using valid Markdown syntax.\n"
        )
        
        # Unified functions list with all capabilities
            functions_spec = [
            # Analytics tools
                {
                    "name": "sql_tool",
        ...
        # Check for email approval/rejection (unified for all queries)
        ...
        # Unified processing - no mode separation needed
                ...
                if fn_calls:
                    ...
                    for call in fn_calls:
                        ...
                        if call["name"] == "sql_tool":
                            ...
                            # Guard against SQL/column errors producing 500s
                            try:
                                result = chart_tool(
                                    args.get("sql", ""),
                            ...
                            # If the provided body has no digits but LAST_BODY has numbers, append recent results
                            import re as _re
...
*Please review the email above and click the Approve button below to send it.*"""
                ...
                if last_chart_snippet:
                    ...
                    LAST_CHARTS = []
                # If model forgot to include the chart markdown, prepend it (broad check for any markdown image)
                if last_chart_snippet and ("<img" not in final_reply and "![" not in final_reply):
                ...
                if mode == "help":
                    ...
                    # 1) collect candidates from top_docs
                    ...
                    # 2) build lightweight keyword set from the query
                    import re
                    tokens = [t for t in re.split(r"[^a-z0-9]+", query.lower()) if len(t) >= 3]
                    stop = {
                        "the","and","for","with","from","that","this","into","how","what",
                        "where","when","rate","cube","help","guide","page","see","link"}
                    keywords = [t for t in tokens if t not in stop]

                    # 3) score candidates by keyword overlap in section_title/content; prefer specific pages
                    def is_specific(url: str) -> bool:
                        trimmed = url.rstrip('/')
                        return (
                            bool(trimmed)
                            and not trimmed.endswith('/help')
                            and not trimmed.endswith('/help/')
                            and not trimmed.endswith('/help/home.html')
                        )
                    ...
                    for url, d in candidates:
                        ...
                        if is_specific(url):
                            score += 1  # small bump for specific pages
                        if score > best_score:
                            best_score = score
                            best_url = url

                    # 4) Validate against whitelist; if candidate not whitelisted, choose best whitelist fallback
                    def choose_from_whitelist(query_text: str) -> str:
                        if not HELP_URL_WHITELIST:
                            return ""
                        import re as _re
                        toks = [t for t in _re.split(r"[^a-z0-9]+", query_text.lower()) if len(t) >= 3]
                        boosts = {
                            "uom": "account.html",
                            "currency": "account.html",
                            "exchange": "exchange-rates.html",
                            "lane": "lane.html",
                            "account": "account.html",
                        }
                        best, bscore = "", 0
                        for url in HELP_URL_WHITELIST:
                            fname = url.split("/")[-1]
                            base = fname.replace(".html", "").replace("-", " ")
                            s = sum(1 for t in toks if t in base)
                            for k, target in boosts.items():
                                if k in toks and fname == target:
                                    s += 2
                            if s > bscore:
                                bscore = s
                                best = url
                    ...
                    if best_url and best_score >= 1 and in_whitelist(best_url):
                        ...
                        chosen = choose_from_whitelist(query)

                    if chosen and chosen not in final_reply:
        ...
        while True:
            ...
            # --------------------------------------------------------------
            fn_calls = []
            if hasattr(msg_choice.message, "function_call") and msg_choice.message.function_call:
                fn_calls = [msg_choice.message.function_call]
            elif hasattr(msg_choice.message, "tool_calls") and msg_choice.message.tool_calls:
                fn_calls = msg_choice.message.tool_calls

            if fn_calls:
                print(f"DEBUG: Processing {len(fn_calls)} function calls")
                for call in fn_calls:
                    print(f"DEBUG: Processing function call: {call.function.name}")
                    args = call.function.arguments if hasattr(call.function, "arguments") else {}
                    if isinstance(args, str):
                        import json
                        args = json.loads(args)

                    if call.function.name == "sql_tool":
                        result = sql_tool(args.get("sql", ""))
                        print(f"DEBUG: sql_tool result: {result[:100]}...")
                    elif call.function.name == "multi_sql_tool":
                        result = multi_sql_tool(args.get("queries", []))
                        print(f"DEBUG: multi_sql_tool result: {result[:100]}...")
                    elif call.function.name == "percentage_tool":
                        result = percentage_tool(
                            args.get("numerator_sql", ""),
                            args.get("denominator_sql", "")
                        )
                        print(f"DEBUG: percentage_tool result: {result[:100]}...")
                    elif call.function.name == "chart_tool":
                        # Guard against SQL/column errors producing 500s
                        try:
                            result = chart_tool(
                                args.get("sql", ""),
                        ...
                        if ("<img" in result) or ("![" in result):
                            last_chart_snippet = result
                    elif call.function.name == "update_dispute_status":
                        result = update_dispute_status(
                            args.get("dispute_id", 0),
                            args.get("new_status", ""),
                            args.get("changed_by", "")
                        )
                        print(f"DEBUG: update_dispute_status result: {result}")
                    elif call.function.name == "add_audit_comment":
                        result = add_audit_comment(
                            args.get("dispute_id", 0),
                            args.get("comments", ""),
                            args.get("processor", ""),
                        ...
                        print(f"DEBUG: add_audit_comment result: {result}")
                    elif call.function.name == "draft_email_tool":
                        result = draft_email_tool(
                            args.get("to_usernames", []),
                            args.get("subject", ""),
                        ...
                        # Store the draft in session storage for approval
                        session_id = "default"  # In a real app, you'd get this from the request
                        EMAIL_DRAFTS[session_id] = {
                            "recipients": args.get("to_usernames", []),
                            "subject": args.get("subject", ""),
                            "body": args.get("body_markdown", ""),
                            "attachments": args.get("attachments", []),
                            "status": "draft"
                        }
                        print(f"DEBUG: Stored EMAIL_DRAFT: {EMAIL_DRAFTS[session_id]}")
                        
                        # Create a formatted email preview
                        email_preview = f"""📧 **Email Draft Ready for Approval**

**To:** {', '.join(args.get("to_usernames", []))}
**Subject:** {args.get("subject", "")}
...
*Please review the email above and click the Approve button below to send it.*"""
                        ...
                        print(f"DEBUG: Formatted email preview: {email_preview[:100]}...")
                        
                        # Return the email draft immediately for user approval
                        return JSONResponse({"reply": email_preview})
                    elif call.function.name == "approve_email_tool":
                        result = approve_email_tool()
                        print(f"DEBUG: approve_email_tool result: {result}")
                    else:
                ...
                # Add help links for all queries (not just help mode)
                if top_docs:
                    # Prefer links strictly from embedded docs and only when the doc actually matches the query.
                    # 1) collect candidates from top_docs
                    candidates: list[tuple[str, dict]] = []
                    ...
                    # Whitelist of valid help pages (from HelpContent/*.html)
                    valid_help_pages = {
                        "http://dev.tcube360.com/help/audit-cube-user-guide.html",
                        "http://dev.tcube360.com/help/rate-cube-user-guide.html",
                        "http://dev.tcube360.com/help/admincube.html",
                        "http://dev.tcube360.com/help/account.html",
                        "http://dev.tcube360.com/help/carrier.html",
                        "http://dev.tcube360.com/help/lane.html",
                        "http://dev.tcube360.com/help/chargemapping.html",
                        "http://dev.tcube360.com/help/fuelsurcharge.html",
                        "http://dev.tcube360.com/help/userprofile.html",
                        "http://dev.tcube360.com/help/role.html",
                        "http://dev.tcube360.com/help/tcube-master.html",
                        "http://dev.tcube360.com/help/email-template.html",
                        "http://dev.tcube360.com/help/exchange-rates.html",
                        "http://dev.tcube360.com/help/invoicedash.html",
                        "http://dev.tcube360.com/help/shipmentdash.html",
                        "http://dev.tcube360.com/help/disputedash.html",
                        "http://dev.tcube360.com/help/ratecalculator.html",
                        "http://dev.tcube360.com/help/ratemaintenance.html",
                    }

                    best_url: str = ""
                    best_score = 0
                    
                    # First, try to find a direct match from candidates that is in the whitelist
                    for url, d in candidates:
                        if url in valid_help_pages:
                            title = (d.get("section_title") or "").lower()
                            content = (d.get("content") or "").lower()
                            hay = f"{title} {content}"
                            score = sum(1 for k in keywords if k in hay)
                    ...
                    # If no direct match or score is low, try to find the best matching page from the whitelist
                    if not best_url or best_score < 1:
                        for url in valid_help_pages:
                            # Simulate content for scoring if not from top_docs
                            simulated_content = url.replace("http://dev.tcube360.com/help/", "").replace(".html", "").replace("-", " ").lower()
                            score = sum(1 for k in keywords if k in simulated_content)
                            
                            # Specific boosts for common queries
                            if any(k in query.lower() for k in ["currency", "uom", "unit of measure", "dimension", "weight"]):
                                if "account.html" in url:
                                    score += 2
                            if any(k in query.lower() for k in ["exchange", "rate"]):
                                if "exchange-rates.html" in url:
                                    score += 2
                            if "lane" in query.lower():
                                if "lane.html" in url:
                                    score += 2
                            if any(k in query.lower() for k in ["ofa", "rfa", "draft rates", "active rates"]):
                                if "ratemaintenance.html" in url:
                                    score += 2
                    ...
                    # Only append link if we found a real match; otherwise, skip to avoid hallucinated links
                    if best_url and best_score >= 1 and best_url not in final_reply:
                        final_reply += f"\n\n[Open related guide]({best_url})"
